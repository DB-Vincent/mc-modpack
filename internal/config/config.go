...
func Load(location string) (*Config, error) {
	...
	// Open file
	file, err := os.ReadFile(fmt.Sprintf("%s/%s", location, ConfigFileName))
	if err != nil {
		return nil, err
	}
...
func Exists(location string) error {
	if _, err := os.Stat(fmt.Sprintf("%s/%s", location, ConfigFileName)); errors.Is(err, os.ErrNotExist) {
